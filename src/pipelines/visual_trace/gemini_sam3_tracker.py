...
def oxe_sample_to_pipeline_input(
    ...
    """
    Convert OXEDataset __getitem__ output -> GeminiSAM3TrackerPipeline expected dict.
...
# Pipeline
# ============================================================
class GeminiSAM3TrackerPipeline(BasePipeline):
    """
    Visual trace pipeline using Gemini for gripper detection refinement + SAM3 video tracking.
    Dataset-agnostic pipeline that uses Gemini to refine gripper bounding boxes.
    """
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run Gemini + SAM3 Tracker pipeline on OXE")
    parser.add_argument("--config", type=str, required=True,
                        help="Config file name (e.g., visual_trace_bridge)")
    ...
    pipeline = GeminiSAM3TrackerPipeline(config)

    print("\nRunning Gemini + SAM3 Tracker pipeline on OXE...")
