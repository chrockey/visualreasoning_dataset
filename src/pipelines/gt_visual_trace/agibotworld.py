...
class GTVisualTraceAgiBotWorldPipeline(BasePipeline):
    """
    GT Visual Trace pipeline for AgiBotWorld dataset.

    Uses camera_params metadata (head_extrinsics, hand_extrinsics, head_intrinsics)
    to project 3D trajectories to 2D pixel coordinates.
    """
    ...
    def process(self, data_dict: Dict[str, Any]):
        ...
        Returns:
            ...
            {
                "head_2d": (T, 2) array,   # Head camera trajectory (usually NaN-filled)
                "hand_2d": (T, 2) array,   # Hand trajectory in pixel coordinates
                "metadata": {
                    "num_frames": int,
                    "video_width": int,
                    "video_height": int,
...
if __name__ == "__main__":
    ...
    config = load_config("gt_visual_trace_agibotworld")
    pipeline = GTVisualTraceAgiBotWorldPipeline(config, verbose=True)
